...
void Jit64::HandleNaNs(UGeckoInstruction inst, X64Reg xmm_out, X64Reg xmm, X64Reg clobber)
{
  ...
  if (inst.OPCD != 4)
  {
    ...
    MOVDDUP(xmm, M(PowerPC::jit_data.ro->psGeneratedQNaN));
  ...
  else
  {
    ...
    if (cpu_info.bSSE4_1)
    {
      ...
      BLENDVPD(xmm, M(PowerPC::jit_data.ro->psGeneratedQNaN));
      for (u32 x : inputs)
      {
    ...
    else
    {
      ...
      ANDPD(tmp, M(PowerPC::jit_data.ro->psGeneratedQNaN));
      ORPD(tmp, R(clobber));
      MOVAPD(xmm, R(tmp));
      for (u32 x : inputs)
      {
...
void Jit64::fmaddXX(UGeckoInstruction inst)
{
  ...
  else
  {
    ...
    if (inst.SUBOP5 == 31)  // nmadd
      XORPD(XMM1, M(packed ? PowerPC::jit_data.ro->psSignBits2 : PowerPC::jit_data.ro->psSignBits));
  }
...
void Jit64::fsign(UGeckoInstruction inst)
{
  ...
  case 40:  // neg
    avx_op(&XEmitter::VXORPD, &XEmitter::XORPD, fpr.RX(d), src,
           M(packed ? PowerPC::jit_data.ro->psSignBits2 : PowerPC::jit_data.ro->psSignBits), packed);
    break;
  case 136:  // nabs
    avx_op(&XEmitter::VORPD, &XEmitter::ORPD, fpr.RX(d), src, M(packed ? PowerPC::jit_data.ro->psSignBits2 : PowerPC::jit_data.ro->psSignBits),
           packed);
    break;
  case 264:  // abs
    avx_op(&XEmitter::VANDPD, &XEmitter::ANDPD, fpr.RX(d), src, M(packed ? PowerPC::jit_data.ro->psAbsMask2 : PowerPC::jit_data.ro->psAbsMask),
           packed);
    break;
  default:
...
void Jit64::fctiwx(UGeckoInstruction inst)
{
  ...
  MOVAPD(XMM0, M(PowerPC::jit_data.ro->half_qnan_and_s32_max));
  MINSD(XMM0, fpr.R(b));
  switch (inst.SUBOP10)
  {
  // fctiwx
