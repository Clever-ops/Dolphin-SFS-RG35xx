...
void retro_set_controller_port_device(unsigned port, unsigned device)
{
  ...
  if (device == RETRO_DEVICE_NONE)
  {
    ...
    {
      Config::SetBase(Config::GetInfoForSIDevice(port), SerialInterface::SIDEVICE_NONE);
      SerialInterface::ChangeDevice(Config::Get(Config::GetInfoForSIDevice(port)), port);
    }
    else if (port > 3)
    {
      Config::SetBase(Config::GetInfoForSIDevice(port - 4), SerialInterface::SIDEVICE_NONE);
  ...
  else if (!SConfig::GetInstance().bWii || device == RETRO_DEVICE_GC_ON_WII || port > 3)
  {
    ...
    Config::SetBase(Config::GetInfoForSIDevice(port > 3 ? port - 4 : port), SerialInterface::SIDEVICE_GC_CONTROLLER);
